    ...
    <div class="drawer drawer-end">
      ...
      <div class="drawer-content">
        ...
        <div class="flex my-4">
          ...
          <label for="customer-drawer" class="btn btn-primary drawer-button">
            ...
            <div v-if="selectedCustomer" class="space-y-4">
              <div class="grid grid-cols-2 gap-4 p-4 my-4">
                ...
                <div class="space-y-4">
                  ...
                  <p>
                    <strong>Province:</strong> {{ selectedCustomer.province }}
                  </p>
                  <p>
                    <strong>Zipcode:</strong> {{ selectedCustomer.zipcode }}
                  </p>
                  <p>
              ...
              <div class="flex mt-6">
                ...
                <button
                  @click="showAddShippingForm = true"
              ...
              <!-- Add Shipping Form -->
              <div
                v-if="showAddShippingForm"
                class="mt-6 p-4 border rounded-lg shadow-md"
              >
                <button
                  @click="showAddShippingForm = false"
                  class="btn btn-sm btn-error mb-4"
                >
                  X
                </button>
                <form @submit.prevent="" class="space-y-4">
                  <div>
                    ...
                    <input
                      type="text"
                      id="receiver_name"
                      v-model="newShippingAddress.receiver_name"
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      id="phone"
                      v-model="newShippingAddress.phone"
                      placeholder="Phone"
                      required
                      class="input input-bordered w-full"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      id="shipping_company"
                      v-model="newShippingAddress.shipping_company"
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      id="Province"
                      v-model="newShippingAddress.province"
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      id="branch_details"
                      v-model="newShippingAddress.branch_details"
                      placeholder="Branch Detail"
                      required
                      class="input input-bordered w-full"
                    />
                  </div>
              ...
              <div
                v-if="
                  selectedCustomer.shipping_information &&
                  selectedCustomer.shipping_information.length > 0
                "
              >
                <table class="w-full table table-zebra mt-2">
                  ...
                  <tbody>
                    <tr
                      v-for="(
                        address, index
                      ) in selectedCustomer.shipping_information"
                      :key="index"
                    >
                      ...
                      <td class="flex items-center">
                        <button
                          @click="deleteShippingID = address.id"
            ...
            <div>
              ...
              <div>
                ...
                <input
                  type="text"
                  id="street_address"
                  v-model="customerObj.street_address"
...
onMounted(() => {
  customerChanel.value = supabase
    .channel("customers-chanel")
    .on(
      "postgres_changes",
...
onUnmounted(() => {
  ...
  if (customerChanel.value) {
    customerChanel.value.unsubscribe();
    customerChanel.value = null;
  }
});
...
const createShippingInfo = async () => {
  ...
  try {
    ...
    useNuxtApp().$toast.error(
      error.message || "An error occurred while adding new shipping info."
    );
  }
};
...
const deleteShippingAddress = async (addressId) => {
  ...
  if (error) {
    useNuxtApp().$toast.error(
      error.message || "Error deleting shipping address."
    );
    return;
  }
  ...
  selectedCustomer.value.shipping_information =
    selectedCustomer.value.shipping_information.filter(
...
const submitCustomerForm = async () => {
  ...
  try {
    ...
    useNuxtApp().$toast.success(
      `New customer has been created: ${data[0].firstname} ${data[0].lastname}`
    );
    if (data) {
      loading.value = false;
